...
import myonset.events as evt
from myonset.utils import utilsfunc
...
def add_limits_events(evts, t0_codes, stim_codes, resp_codes, start_code, end_code, channel=-1,\
    ...
    of trial event is detected using 'idx_end_event' function as: 1) the event
    preceding the occurence of next start_code, 2) the event preceding the
    ...
    or 3) occured within  duration after t0_code event (if maximum duration tmax
    is provided).
...
def decode_accuracy(dataframe, correct_resp, resp_chan_dict,\
    ...
    'decode_trial_accuracy' or 'decode_trial_accuracy_resp_codes'. Returns 
    dataframe, with new columns containing response and channel onset(s)/offset(s)
    accuracy. Channel onset(s)/offset(s) are also sorted as responding 
    (associated with the response occurrence),and non responding (not associated 
    with the response).
    ...
    dataframe : class pandas.DataFrame
        ...
        offset channel(s).
    correct_resp : dict | list
        Dictionnary with possible stimulus codes as keys, and corresponding
        correct response codes as values OR list of correct resp codes.
    resp_chan_dict : dict
    ...
    for index, trial in dataframe.iterrows():
        ...
        if isinstance(correct_resp,dict):
            accuracy, onset_correct, offset_correct, onset_incorrect, offset_incorrect = decode_trial_accuracy(stim, resp, onset_time, onset_chan,\
                                                                                                               offset_time, offset_chan, correct_resp, resp_chan_dict)
        else:
            accuracy, onset_correct, offset_correct, onset_incorrect, offset_incorrect = decode_trial_accuracy_resp_codes(stim, resp, onset_time, onset_chan,\
                                                                                                                          offset_time, offset_chan, correct_resp, resp_chan_dict)
...
def decode_trial_accuracy_resp_codes(stim_code, resp_code, onset_time, onset_chan, offset_time, offset_chan,\
                                     correct_resp, resp_chan_dict):
    """Decodes trial channel (e.g., EMG) accuracy, knowing correct response code.
    ...
    offset_chan: list
        List containing corresponding channel(s).
    correct_resp : list
        List with possible correct response codes.
    resp_chan_dict: dict
        Dictionnary with response codes as keys, and corresponding channels
        as values.
    ...
    if resp_code in correct_resp:
        correct = resp_code
    else: correct = correct_resp[0] # anything but not the actual response
    ...
    for c in range(len(onset_chan)):
        if (onset_chan[c] == resp_chan_dict[correct]) and (accuracy == 'correct'):
            onset_correct.append(onset_time[c])
        elif onset_chan[c] in resp_chan_dict.values():
            onset_incorrect.append(onset_time[c])
    for c in range(len(offset_chan)):
        if (offset_chan[c] == resp_chan_dict[correct]) and (accuracy == 'correct'):
            offset_correct.append(offset_time[c])
        elif offset_chan[c] in resp_chan_dict.values():
            offset_incorrect.append(offset_time[c])
